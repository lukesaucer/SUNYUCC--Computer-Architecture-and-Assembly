...
# and down.  (Notice that the y direction is the opposite of math tradition.)
 	    ...
 	    li $a0,200    			# left x coordinate
            ...
            li $a1,50     			# width
            li $a2,200    			# top y coordinate (y increases downward)
            li $a3,25     			# height
            jal rectangle
...
# $a3 is height (must be nonnegative and within the display)
            ...
            beq $a1,$zero,rectangleReturnFill 	 # zero width: draw nothing
            beq $a3,$zero,rectangleReturnFill 	 # zero height: draw nothing
            lw $t0, colorFill
            la $t1,frameBuffer
            add $a1,$a1,$a0              	 # simplify loop tests by switching to first too-far value
            add $a3,$a3,$a2
...
rectangleXloopFill:
            sw $t0,($t3)
            addiu $t3,$t3,4
            bne $t3,$t2,rectangleXloopFill       # keep going if not past the right edge of the rectangle
            addu $a2,$a2,$t4                 	 # advace one row worth for the left edge
            addu $t2,$t2,$t4                	 # and right edge pointers
            bne $a2,$a3,rectangleYloopFill       # keep going if not off the bottom of the rectangle
